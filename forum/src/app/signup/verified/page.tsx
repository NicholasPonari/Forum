...
function VerifiedSignUpPageContent() {
  ...
  const [blockchainStatus, setBlockchainStatus] = useState<
    "pending" | "issuing" | "success" | "failed" | null
  >(null);
  ...
  const onSubmit = async (data: SignUpFormValues) => {
    ...
    try {
      ...
      // Issue blockchain identity (non-blocking: signup succeeds even if this fails)
      setBlockchainStatus("issuing");
      try {
        if (finalUserId) {
          const bcResponse = await fetch("/api/blockchain/issue-identity", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              userId: finalUserId,
          ...
          if (bcResponse.ok) {
            ...
            setBlockchainTxHash(bcResult.txHash);
          } else {
            console.warn(
              "[Blockchain] Identity issuance failed, will retry later",
            );
            setBlockchainStatus("failed");
          }
        }
      } catch (bcError) {
        console.warn("[Blockchain] Identity issuance error:", bcError);
        setBlockchainStatus("failed");
      }
                          ...
                          <div>
                            ...
                            {blockchainTxHash && (
                              <p className="text-white/50 text-xs mt-1 font-mono truncate">
                                tx: {blockchainTxHash.slice(0, 18)}...
                              </p>
                            )}
                          </div>
                        )}
                  ...
                  <ul className="space-y-2 text-gray-700 text-sm">
                    ...
                    {blockchainStatus === "success" && (
                      <li className="flex items-start gap-2">
                        ...
                        <span>
                          Identity cryptographically recorded on blockchain
                        </span>
                      </li>
                    )}
                  </ul>
